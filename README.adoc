...
The client itself consists of a class in the file:///./mailpit/client/api.py[API-module], that offers methods, which query the API-Endpoints and are named respectively. +
...
messages = api.get_messages()
```
messages will be an instance of the `Messages` class, which you can find in file:///./mailpit/client/models/messages.py[mailpit/client/models/messages.py]. +
...
For examples have a look at the file://./tests[tests]
